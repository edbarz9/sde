...
browse(char *ipath, char *ifilter)
{
  ...
  char oldfullp[PATH_MAX], newfullp[PATH_MAX];
	char fltr[LINE_MAX];
...
nochange:
    ...
    case SEL_RENAME:
		  ...
		  tmp = readln();
      if (tmp != NULL){
      strlcpy(oldfullp,path,sizeof(path));
      strlcat(oldfullp,"/",sizeof(oldfullp));
      strlcat(oldfullp,dents[cur].name,sizeof(oldfullp));
      strlcpy(newfullp,path,sizeof(path));
      strlcat(newfullp,"/",sizeof(newfullp));
      strlcat(newfullp,tmp,sizeof(newfullp));
      renames = rename(oldfullp, newfullp);
			goto begin;
      } else {
				printmsg("File needs a name");
				goto nochange;
      }
      break;
		}
